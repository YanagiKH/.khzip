...
pub fn create_archive(options: &CreateOptions) -> Result<ArchiveSummary> {
    ...
    let effective_mode = if matches!(
        options.format,
        ArchiveFormat::Khaz | ArchiveFormat::Khcz
    ) {
    ...
    for source in entries {
        ...
        stream_chunks(&mut reader, chunk_config, |chunk| {
            ...
            ids.push(id);
            if !chunk_lookup.contains_key(&id) {
...
pub fn extract_archive(
    ...
    with_joined_archive(path, |actual| {
        ...
        let chunks: HashMap<[u8; 32], ChunkIndex> = manifest
            .chunks
            .iter()
            .cloned()
...
fn split_archive(source: &Path, output: &Path, split_size: u64) -> Result<()> {
    let base = if output
        .extension()
        .and_then(|extension| extension.to_str())
        .is_some_and(|extension| extension.eq_ignore_ascii_case("khx"))
    {
        output.to_path_buf()
    } else {
...
fn is_split_part(path: &Path) -> bool {
    path.extension()
        ...
        .is_some_and(|extension| {
            extension.len() == 3 && extension.chars().all(|character| character.is_ascii_digit())
        })
}
